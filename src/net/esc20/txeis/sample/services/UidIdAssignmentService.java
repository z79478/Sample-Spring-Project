...
public class UidIdAssignmentService extends UidXmlService {
	...
	public SOAPMessage createRequest(UIDModel model) throws Exception {
		...
		SOAPElement assignId = body.addChildElement("wsv2:assignId");
		assignId.setAttribute("soapenv:encodingstyle", "http://schemas.xmlsoap.org/soap/encoding/");
		
		SOAPElement student = assignId.addChildElement("student");
		...
		SOAPElement userid = assignId.addChildElement("userId");
		userid.setAttribute("xsi:type", "xsd:string");
		userid.addTextNode(UIDModel.USERID);
			
		SOAPElement password = assignId.addChildElement("password");
